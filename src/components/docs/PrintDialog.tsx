...
export function PrintDialog() {
    ...
    const downloadBack = useButtonAction(async () => await ref.current?.download("back") ?? null)
    const downloadFront = useButtonAction(async () => await ref.current?.download("front") ?? null)
    ...
    const { docKey, doc, validId } = hashDoc
    ...
    const state_1 = !validId
        ...
        : doc
            ? "doc-ready"
            : "doc-not-uploaded";
    const state_2 = state_1 !== "doc-ready" ? "hidden" : verified ? "verified" : "verify";
    const state_3 = state_2 !== "verified" ? "hidden" : downloadBack.lastRun && downloadFront.lastRun ? "downloaded" : "download";
    const state_4 = state_3 !== 'downloaded' ? 'hidden' : loggedIn ? 'loggedIn' : 'login';
        ...
        <>
            ...
            {state_2 === "hidden" ? null : (
                <section>
                    ...
                    <CardDisplay {...{ docKey, json: doc.data, link: link!, ref }} />
                    <p>
                        <input type="checkbox" onChange={({ currentTarget: { checked } }) => setVerified(checked)}></input>
                    </p>
                </section>
            )}
            {state_3 === "hidden" ? null : (
                <section>
                    ...
                    <p>
                        ...
                        <button onClick={downloadFront} disabled={downloadFront.isRunning}>Front Side</button>
                        {
                        ...
                        <button onClick={downloadBack} disabled={downloadBack.isRunning}>Back Side</button>
                    </p>
                </section>
            )}
            {state_4 === 'hidden' ? null : (
                <section>
                    ...
                    <p>
                        You will need a <a href="https://raksul.com">Raksul</a> account for this step. → <a href="https://raksul.com/account/login" target="_blank">Login&#x2F;Signup</a>
                    </p>
                    <p>
                        Open the products editor for business card and select an the empty template.
                    ...
                    <p>
                        Drag & Drop the front side on the editor and use the "配置" button to change the positon.
                        Set it to 96x60 at -2.5/-2.5.
                    ...
                    <p>
                        Now go to order a regular business card. Select the "通常" (tsujo) business cards.

                        <a target="_blank" href="https://raksul.com/print/businesscard/prices?size=STANDARD_BUSINESSCARD&paper=MATTE&weight=220&color=BOTH_SIDE_COLOR&rounded=ROUNDED_OFF&coating=COATING_NONE" onClick={() => setLoggedIn(true)}>Take me there!</a>
                    </p><p>
                        Use the quality "標準"(hyojun) for and we need it colored on both sides.

                        Note: if you are not in a hurry it is no problem to order the business cards a few days in advance. they then cost much less!

                        <img src="/assets/screenshot-02-add-to-cart.png" />
                        <img src="/assets/screenshot-03-selection.png" />
                    </p>
                    <p>
                        To upload the designs use the <img src="/assets/screenshot-04-upload-data.png" /> and <img src="/assets/screenshot-05-select-mydesign.png" /> to add our designs.
                    </p>
                    <p>
            ...
            {state_5 === 'hidden' ? null : (
                <section>
                    <h2>❻ Select your design</h2>
                        <input type="checkbox" onChange={({ currentTarget: { checked } }) => setMyDesign(checked)}></input>
                </section>
            )}
        </>
    );
