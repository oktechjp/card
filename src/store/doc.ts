import { createPrivateKey, fetchDocument, isPossibleDocKey } from "@/utils/safeDoc"
...
const persistentIds = persistentMap<Record<string, boolean>>('ids', {}, {
    encode: bool => bool ? '1' : '0',
    decode: str => str === '1'
})

const persistentDrafts = persistentMap<Record<string, any>>('drafts', {}, {
...
export const knownDraftIds = computed([persistentDrafts], (all) => Object.keys(all))
...
export const docs = mapCreator<DocStore<any>>(
    (store, id) => {
        ...
        const refresh = () => {
            store.set({ ...store.get(), saveDraft, discard, state: 'loading', draft: persistentDrafts.get()[id] })
            task(async () => {
                try {
                    const doc = await fetchDocument(id)
                    const current = store.get()
                    const isDirty = current.draft ? !deepEqual(current.draft, doc.data) : false
                    ...
                    console.warn(err)
                    const current = store.get()
                    store.set({ ...current, saveDraft, discard, state: 'ready', refresh, doc: null, isDirty: !!current.draft })
                }
            })
        }
        refresh()
