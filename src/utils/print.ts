...
export async function openSvgImage(elem: SVGSVGElement, opts?: SVGToImageProps) {
    if (!globalThis.open) {
...
export async function downloadSvgImage(elem: SVGSVGElement, name: string, opts?: SVGToImageProps) {
    const doc = globalThis.document
    if (!doc) {
