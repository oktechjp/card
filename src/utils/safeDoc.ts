import { decodeJSON, encode, encodeJSON, fromBase64, toBase64, toCrockfordBase32 } from "./buffer"
import type { JSONObj } from "./form"

const ENCRYPT_ALGO = "AES-GCM"
const ENCRYPT_LEN = 256
const PAGE_SALT = new Uint8Array([46, 55, 162, 142, 175, 124, 103, 38, 133, 107, 147, 85])
const PAGE_KEY = new Uint8Array([226, 196, 13, 158, 36, 191, 211, 94, 53, 177, 201, 188])
...
async function createKey(secret: string | Uint8Array<ArrayBuffer>) {
    ...
    const deriveKey = await crypto.subtle.importKey("raw", secret, "PBKDF2", false, ["deriveKey"])
...
export async function decryptDocument(privateKey: string, json: EncryptedDocumentWrapper) {
...
export async function encryptDocument(privateKey: string, type: string, version: number, data: JSONObj): Promise<{
    ...
    return {
        ...
        link: `${LINK_PREFIX}${privateKey}`,
        encrypted: {
            content: toBase64(await new Uint8Array(await crypto.subtle.encrypt(
                { name: ENCRYPT_ALGO, iv: PAGE_SALT },
                encryptionKey,
                encodeJSON(json)
            )))
